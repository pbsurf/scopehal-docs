...
URI. Radios with two receive paths show up as two complex channels.

The center frequency sets the receive local oscillator and the span sets the analog RF bandwidth. The sample rate is
configured separately, and should be at least as wide as the span. If the radio rejects a setting (for example, the
AD9363 in a stock PlutoSDR cannot tune below 325 MHz) the value snaps back to what the radio is actually doing.

Some things to be aware of:
\begin{itemize}
\item Only receive is supported. Gain is left at whatever the radio is currently configured for (a PlutoSDR
defaults to automatic gain control) and cannot yet be changed from ngscopeclient.
...
915~MHz and 2.4~GHz bands, which move around as you retune it.
