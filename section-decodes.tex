...
\pagebreak
\section{Complex FFT}
\label{filter:complexfft}

Calculates a Fast Fourier Transform of complex I/Q data, such as the output of a software defined radio, and displays
the log magnitude response.

This is the complex input equivalent of the \hyperref[filter:fft]{FFT} filter. Because the input is complex, the
spectrum is two-sided: the output has as many points as the input and covers the entire sample rate, from half the
sample rate below the center frequency to half above it. The horizontal axis of the output is absolute frequency.

The window functions and peak detection work the same way as for the FFT filter, and the amplitude scaling is the same
as the \hyperref[filter:complexspectrogram]{Complex Spectrogram}, so the two show the same level for the same signal.
...
\thinhline
center & Analog scalar & Frequency that the zero point of the complex waveform is at\\
\thickhline
\end{tabularx}

I and Q must have the same sample rate.
...
This filter outputs an analog waveform of power in dBm versus frequency.
...
\label{filter:complexspectrogram}
